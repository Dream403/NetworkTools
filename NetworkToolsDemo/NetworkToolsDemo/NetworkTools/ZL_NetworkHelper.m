...
static ZL_NetworkResponseType _responseType = ZL_NetworkResponseTypeJSON;

static ZL_NetworkRequestType  _requestType  = ZL_NetworkRequestTypePlainText;
...
static ZL_NetworkStatus  networkStatus;
...
@implementation ZL_NetworkHelper
/**
 *  监听网络
 */
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown:
                _status ? _status(ZL_NetworkStatusUnknown) : nil;
                _isNetwork = NO;
                NSLog(@"未知网络");
                break;
            case AFNetworkReachabilityStatusNotReachable:
                _status ? _status(ZL_NetworkStatusNotReachable) : nil;
                _isNetwork = NO;
                NSLog(@"无网络");
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:{
                //延迟0.5秒，等待状态栏改变后再获取
                ZL_NetworkStatus status =   [self currentNetWorkStatus];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    _status ? _status(status) : nil;
                    _isNetwork = YES;
                });
        
                break;}
            case AFNetworkReachabilityStatusReachableViaWiFi:
                _status ? _status(ZL_NetworkStatusReachableViaWiFi) : nil;
                _isNetwork = YES;
                NSLog(@"WIFI");
                break;
        }
...
+ (NSURLSessionDataTask *)uploadWithURL:(NSString *)URL
    ...
    if (networkStatus == ZL_NetworkStatusNotReachable) {
...
+ (NSURLSessionDataTask *)WithMethodType:(HTTPMethodType) type  isCache:(BOOL) isCache URL:(NSString *)URL
   ...
   //获取缓存
   responseCache ? responseCache([ZL_NetworkCache getHttpCacheForKey:URL]) : nil;
    ...
    switch (type) {
        ...
        case HTTPMethodTypeGET:{
           ...
           task =  [manger GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
                ...
                if (isCache) {
                    [ZL_NetworkCache saveHttpCache:responseObject forKey:URL];
                }
...
+ (void)successResponse:(id)responseData complete:(ZL_NetworkRequestSuccess)success {
...
+(AFHTTPSessionManager *)manager{
    ...
    ZL_SessionManger *manager =  [ZL_SessionManger manager];
...
//     [manager setSecurityPolicy:[self customSecurityPolicy]];
    ...
    switch (_requestType) {
            //请求头格式默认
        case ZL_NetworkRequestTypePlainText:
        {
            manager.requestSerializer = [AFHTTPRequestSerializer serializer];

            break;
        }
        case ZL_NetworkRequestTypeJSON:
        {
    ...
    switch (_responseType) {
        case ZL_NetworkResponseTypeJSON:
        {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];

            break;
        }
        case ZL_NetworkResponseTypeXML:
        {
        ...
        case ZL_NetworkResponseTypeData:
        {
...
//获取网络状态
+(ZL_NetworkStatus )currentNetWorkStatus
{
