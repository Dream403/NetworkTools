...
//  ChargingPile
//
//  Created by ZL on 16/9/13.
//  Copyright © 2016年 LittleShrimp. All rights reserved.
//

#import "ZL_SessionManger.h"
//防止循环引用
#define CAWeakSelf(type)  __weak typeof(type) weak##type = type;

#define CAStrongSelf(type)  __strong typeof(type) type = weak##type;
static NSString *p12file = @"client";
static NSString *p12pwd = @"haha";
static NSString *serice = @"haha";
@implementation ZL_SessionManger
...
[self setSessionDidReceiveAuthenticationChallengeBlock:^NSURLSessionAuthChallengeDisposition(NSURLSession * _Nonnull session, NSURLAuthenticationChallenge * _Nonnull challenge, NSURLCredential *__autoreleasing  _Nullable * _Nullable credential) {
    ...
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        if (![fileManager fileExistsAtPath:p12]) {
            ...
            if ([ZL_SessionManger extractIdentity:&identity andTrust:&trust fromPKCS12Data:pkcs12Data]) {
